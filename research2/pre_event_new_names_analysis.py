...
OUT=ROOT/'pre_event_new_names_combined';OUT.mkdir(exist_ok=True)

def main():
    c=pd.concat([pd.read_pickle(ROOT/f'pre_event_new_names_results_{y}/candidates.pkl').assign(year=y) for y in range(2022,2026)],ignore_index=True)
    c=c[c.kind.eq('single')].copy();c['signal_spot']=np.nan
    for year,g in c.groupby('year'):
        stock=pd.read_pickle(ROOT/f'pre_event_new_names_data_{year}/stocks.pkl');stock.date=pd.to_datetime(stock.date);stock=stock.set_index(['secid','date'])
        ...
        for version in ['baseline','liquid_cost','entry_limit']:
            ...
            for hedge in ['none','signal','entry']:
                for frac in [0,.25,.5,1]:
                    ...
                    trades.append(z)
                    for period,years in [('development',list(range(2016,2022))),('later',list(range(2022,2026))),('all',list(range(2022,2026)))]:
...
'''
    ...
    path=ROOT.parent/'event-clock/docs/PRE_EARNINGS_NEW_NAMES.md';path.write_text(text)
